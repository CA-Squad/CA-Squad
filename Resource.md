# Resources

- [Introducing the CAKE framework - Frontier Research](https://frontier.tech/the-cake-framework)

CAKE stands for Chain Abstraction Key Elements and it introduces a three-layer thinking framework which breaks down CA into permission, solver and settlement layers.

- [Introducing OneBalance - Frontier Research](https://frontier.tech/onebalance)

- [Intent-Based Architectures and Their Risks - Paradigm](https://www.paradigm.xyz/2023/06/intents)

- [Order flow, auctions and centralisation I: a warning - Flashbots](https://collective.flashbots.net/t/order-flow-auctions-and-centralisation-i-a-warning/258)

- [A decentralised solver architecture for executing intents on EVM blockchain - Rishotics](https://ethresear.ch/t/a-decentralised-solver-architecture-for-executing-intents-on-evm-blockchain/16608)

- [MagicSpend++ Spend Now, Debit Later](https://ethresear.ch/t/magicspend-spend-now-debit-later/19678/12)

- [EIP-7377: Migration Transaction Allow EOAs to send a one-time transaction which deploys code at their account](https://eips.ethereum.org/EIPS/eip-7377)

- [How simple choices lead down the complex path to ERC-4337 - Alchemy](https://www.alchemy.com/blog/account-abstraction)
